import { v4 as uuidv4 } from "uuid";

import { eventIcons } from "../data/icons";

// x: 343, y: 6

const timelineData = {
    [-298655]: [
        {
            id: uuidv4(),
            book: 3,
            name: "Start of book 3 - 33rd Jaghut War",
            icon: eventIcons.tlanimass,
            description: "",
            char: [],
            loc: [522.65, 136.47],
            moon: [0, 0, 0],
        },
    ],
    [-119736]: [
        {
            id: uuidv4(),
            book: 3,
            name: "3 years after the Fall of the Crippled God",
            icon: eventIcons.warren,
            description: "3 years after the Fall of the Crippled God",
            char: [14, 29],
            loc: [315.06, 251.73],
            moon: [0, 0, 0],
        },
    ],
    1154: [
        {
            id: uuidv4(),
            book: 1,
            name: "Start of book 1 - Laseen becomes empress",
            icon: eventIcons.city,
            description: "",
            char: [1, 5, 19],
            loc: [420.0, 185.59],
            moon: [0, 0, 0],
        },
    ],
    1160: [
        {
            id: uuidv4(),
            book: 1,
            name: "Siege of Pale",
            icon: eventIcons.battle,
            description: "",
            char: [3, 4, 5, 6, 7, 8, 11, 12, 14],
            loc: [520.8, 75.0],
            moon: [179, 5, 70],
        },
    ],
    1161: [
        {
            id: uuidv4(),
            book: 1,
            name: "Sorry is born",
            icon: eventIcons.assassin,
            description: "Sorry is born, Lorn and Parnn investiage, Sorry enlists in nearby garrison",
            char: [1, 2, 17],
            loc: [391.67, 179.45],
            moon: [179, 5, 70],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Paran travels the imperial warren",
            icon: eventIcons.warren,
            description: "Topper brings Paran through the imperail warren to Unta infront of Empress Lassen and then returns home to see his sister Tavore",
            char: [1, 10, 17],
            loc: [415.57, 165.64],
            moon: [179, 5, 70],
        },
    ],
    1163: [
        {
            id: uuidv4(),
            book: 1,
            name: "Conquest of Pale",
            icon: eventIcons.battle,
            description: "",
            char: [2, 3, 4, 5, 6, 7, 8, 11, 12, 14],
            loc: [520.88, 75.07],
            moon: [179, 5, 70],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Paran arrives on Genabackis",
            icon: eventIcons.travelHorse,
            description: "",
            char: [1],
            loc: [517.34, 35.16],
            moon: [179, 5, 70],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Paran arrives at Pale as captain of the Bridgeburners",
            icon: eventIcons.death,
            description: "",
            char: [1, 2, 3, 4, 5, 6, 7, 11, 12, 13],
            loc: [520.88, 75.07],
            moon: [178.08, 4, 79.82],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Darujhistan",
            icon: eventIcons.city,
            description: "",
            char: [15, 16, 14],
            loc: [522.65, 84.28],
            moon: [522.655, 4, 79.82],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Bridgeburners arrive north of Darujhistan",
            icon: eventIcons.travelBoat,
            description: "the Bridgeburners find a boat to sail accross to Darujhistan where Sorry reveals she knows how to sail.",
            char: [2, 4, 5, 11, 12],
            loc: [522.65, 81],
            moon: [178.08, 4, 79.82],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Adjunct Lorn arrives at Pale",
            icon: eventIcons.city,
            description: "Lorn confronts Tattersail on her killing her family",
            char: [1, 3, 6, 7, 13, 17, 18],
            loc: [520.88, 75.07],
            moon: [178.08, 4, 79.82],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Crone meets Caladin brood",
            icon: eventIcons.travelFly,
            description: "",
            char: [14, 9],
            loc: [530.62, 52.05],
            moon: [178.08, 4, 79.82],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Tattersail opens her warren",
            icon: eventIcons.warren,
            description: "Tattersail opens her warren but due to being near tool it explodes killing her. Paran and Toc finder her reamins the next day",
            char: [3, 17, 18, 1, 7],
            loc: [530.62, 79.67],
            moon: [530.62, 4, 79.82],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Crone is attacked",
            icon: eventIcons.travelFly,
            description: "Hairlock attacks Crone and other crows",
            char: [6, 14],
            loc: [528.85, 76.61],
            moon: [178.08, 4, 79.82],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Gadrobi Hills - Quest",
            icon: eventIcons.sleeping,
            description: "Lorn and Tool find the barrow for the Jaghut Tyrant",
            char: [17, 18, 2, 15, 16, 1, 6, 7, 8, 14, 3],
            loc: [529.73, 84.28],
            moon: [178.08, 4, 79.82],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Return to Darujhistan",
            icon: eventIcons.city,
            description: "Everyone makes their way back to Darujhistan and arrive at various times.",
            char: [1, 2, 4, 5, 8, 11, 12, 14, 15, 16],
            loc: [522.65, 84.28],
            moon: [178.08, 4, 79.82],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Gadrobi Hills - Jaghut Tyrant",
            icon: eventIcons.tlanimass,
            description:
                "The Jaghut Tyrant's Raest awakens and fights 4 black dragons and the red dragon Silanah. Raest enters Kruppes dream and meets K'rul, Tool destorys his body but he escapes into a new body ",
            char: [],
            loc: [529.73, 84.28],
            moon: [178.08, 4, 78.82],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Battle over Darujhistan",
            icon: eventIcons.seed,
            description: "The Finnest grows into a monster, and the Tyrant attacks the city",
            char: [1, 2, 4, 5, 8, 11, 12, 14, 15, 16, 17, 18],
            loc: [522.65, 84.28],
            moon: [178.6, 2.5, 78.3],
        },
        {
            id: uuidv4(),
            book: 1,
            name: "Return home",
            icon: eventIcons.travelBoat,
            description: "Returning Sorry home",
            char: [2, 11, 12, 15],
            loc: [517.34, 81.14],
            moon: [184.08, 5, 79.82],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Start of book 2 - The cull",
            icon: eventIcons.death,
            description: "The Nobles of Unta are culled and Felisin is enslaved by her sister.",
            char: [19, 20, 21, 22],
            loc: [415.57, 165.64],
            moon: [179, 0, 70],
        },
    ],
    1164: [
        {
            id: uuidv4(),
            book: 3,
            name: "Crater",
            icon: eventIcons.travelCart,
            description: "A caravan of merchants heading eastward towards Darujhistan are blocked by a river whose bridge was destroyed,",
            char: [30, 31, 32],
            loc: [529.73, 84.28],
            moon: [182.31, 0, 82.89],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Convergence?",
            icon: eventIcons.ruins,
            description: "Soultaken are searching for something in the desert",
            char: [23, 24],
            loc: [63.72, 52.19],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 3,
            name: "The tower",
            icon: eventIcons.warren,
            description: "Toc the Younger wakes up after being spat out of the warren, Tool finds him and they make their way to Lady Envy's tower",
            char: [7, 18, 33],
            loc: [179.655, 130.475],
            moon: [182.31, 0, 82.89],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Coltain arrives",
            icon: eventIcons.travelHorse,
            description: "Coltain is made commander of the 7th Malazan army",
            char: [25, 26, 27],
            loc: [81.42, 52.19],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Offpage Journey",
            icon: eventIcons.travelBoat,
            description: "The joureny to return Sorry home from book 1 to book 2",
            char: [2, 11, 12, 15],
            loc: [84.075, 19.955],
            moon: [179, 0, 70],
        },

        {
            id: uuidv4(),
            book: 2,
            name: "Soletaken at sea",
            icon: eventIcons.diverOcean,
            description: "Fiddle explodes a Soletaken while at sea",
            char: [2, 11, 12, 15],
            loc: [80.535, 29.165],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "The holy book",
            icon: eventIcons.prayer,
            description: "Kalam takes the holy book and Fiddler goes his seperate way in order to complete their intitial goal.",
            char: [2, 11, 12, 15],
            loc: [65.49, 42.98],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Panpot'sun Odhan",
            icon: eventIcons.ruins,
            description: "Mappo and Icarium meet Iskaral Pust and serverant.",
            char: [23, 24, 28],
            loc: [61.95, 46.05],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Slave",
            icon: eventIcons.slave,
            description: "Felisin sells her body to try and make a better life for Herboric and Baudin even though they dont care.",
            char: [19, 20, 21],
            loc: [90.27, 46.05],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 3,
            name: "Rivals",
            icon: eventIcons.rally,
            description: "Everyone meets together to discuss the attack on the Pannion Domin",
            char: [1, 3, 4, 5, 8, 9, 13, 14, 16, 29],
            loc: [180.54, 67.53],
            moon: [182.31, 0, 82.89],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "War games",
            icon: eventIcons.battle,
            description: "Coultain carrys out mock battles to prepare the Malazan army to protect civilians in a time of war, much to their distain.",
            char: [25, 26, 27],
            loc: [81.42, 52.19],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Tesem Temple",
            icon: eventIcons.ruins,
            description: "Mappo and Icarium explore.",
            char: [23, 24, 28],
            loc: [61.95, 46.05],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Solo",
            icon: eventIcons.assassin,
            description: "Kalam rests from his travels but is pursed by the Red Blade Captain Lostara Yil.",
            char: [11],
            loc: [59.295, 47.585],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "The Ritual",
            icon: eventIcons.warren,
            description: "Duiker and Kulp are attacked by D'ivers.",
            char: [25, 27],
            loc: [80.535, 50.65],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "The poor disguise",
            icon: eventIcons.fete,
            description: "Fiddlers group pretend to be newly weds in order to travel the 3 citys and the group work out that Dancer is now Cotillion.",
            char: [2, 12, 15],
            loc: [62.835, 50.65],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Sha'ik",
            icon: eventIcons.prayer,
            description: "Kalam hands the holy book over to Sha'ik and leaves with the Aptorian. Sha'ik is assisanated",
            char: [11],
            loc: [55.75, 47.58],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "The Whirlwind begins",
            icon: eventIcons.whirlwind,
            description: "The Whirlwind begins, which casues Duiker and Kulp to be split up from each other and Colatines warband.",
            char: [25, 27],
            loc: [80.535, 50.65],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Army of the Apocalypse",
            icon: eventIcons.rally,
            description: "Duiker returns to Hissar to see the chaos and pretends to be member of the Apocalypse with a deathwish of revenge, Coltaine is attemping to bring the refugses back to Aran",
            char: [25],
            loc: [81.42, 52.19],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Into the Whirlwind",
            icon: eventIcons.whirlwind,
            description: "Fiddler's Crew the great Whirlwind to escape pursuit.",
            char: [2, 12, 15],
            loc: [57.525, 47.585],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Unexpected saviour",
            icon: eventIcons.assassin,
            description: "Kalam is attacked by D'ivers but the demon Apt protects him but sustains damage.",
            char: [11],
            loc: [55.755, 50.65],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Hope?",
            icon: eventIcons.uprising,
            description: "Duiker returns to Hissar to see the chaos and pretends to be member of the Apocalypse with a deathwish of revenge, Coltaine is attemping to bring the refugses back to Aran.",
            char: [25],
            loc: [79.65, 52.19],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Freedom?",
            icon: eventIcons.travelFoot,
            description: "The group travels the desert and almost die but the Boar cult god Fener heals Herboric but is brought into the mortal world.",
            char: [19, 20, 21],
            loc: [88.5, 42.98],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Happenstance",
            icon: eventIcons.diverLand,
            description: "Mappo and Icarium save Fiddlers group.",
            char: [2, 12, 15, 23, 24],
            loc: [57.525, 47.585],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Scorched earth",
            icon: eventIcons.travelHorse,
            description: "Duiker is in pursuit of the Chain of Dogs, and is impressed by how well Coltaine is doing.",
            char: [25],
            loc: [76.995, 53.724],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Saftey or death?",
            icon: eventIcons.warren,
            description: "Felisins group are on their last legs, and then Kulp appears asking for their help from an insane mage who brings them into an elder warren.",
            char: [19, 20, 21, 27],
            loc: [84.96, 42.98],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Tesem Temple",
            icon: eventIcons.ruins,
            description: "Fiddler recovers and Aspala leaves with her father and the group go after her.",
            char: [23, 24, 28, 2, 12, 15],
            loc: [61.95, 46.05],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Kalam, the Kalam?",
            icon: eventIcons.assassin,
            description: "Kalam saves a group from bandits and join forces with them heading for Aren.",
            char: [11],
            loc: [55.755, 53.72],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Chain of Dogs - I",
            icon: eventIcons.dog,
            description: "Duiker rejoins Coltaine army.",
            char: [25, 26],
            loc: [76.995, 53.724],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Back to the desert?",
            icon: eventIcons.warren,
            description: "Felisins group and Kulp escape the warren and end up on the mainland.",
            char: [19, 20, 21, 27],
            loc: [56.64, 46.05],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Imperial Warren",
            icon: eventIcons.warren,
            description: "Kalam group go into the Imperial warren aiming for Aren.",
            char: [11],
            loc: [55.755, 53.72],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Chain of Dogs - II",
            icon: eventIcons.dog,
            description: "Duiker joins a skirmish on the enemy camp.",
            char: [25, 26],
            loc: [63.72, 58.33],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "The First Empire",
            icon: eventIcons.ruins,
            description: "Felisins group find remnants of the first empire.",
            char: [19, 20, 27],
            loc: [56.64, 46.05],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Tracking",
            icon: eventIcons.whirlwind,
            description: "Fiddler groups trys to find Apsalar and her father.",
            char: [23, 24, 28, 2, 12, 15],
            loc: [57.525, 47.585],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Chain of Dogs - III",
            icon: eventIcons.dog,
            description: "Duiker joins a skirmish on the enemy camp.",
            char: [25, 26],
            loc: [63.72, 58.33],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Chain of Dogs - IV",
            icon: eventIcons.dog,
            description: "Battle of Gelor Ridge.",
            char: [25, 26],
            loc: [60.18, 58.33],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Exit",
            icon: eventIcons.city,
            description: "Kalam group exit the Imperial at Aren.",
            char: [11],
            loc: [47.79, 79.82],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Protector",
            icon: eventIcons.death,
            description: "Felisins group leave the city but are attacked by the Rat D'ivers, Kulp and Baudin die.",
            char: [19, 20, 21, 27],
            loc: [55.755, 47.585],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Set sail",
            icon: eventIcons.travelBoat,
            description: "Kalam is on the boat on board to Malaz City.",
            char: [11],
            loc: [61.95, 72.145],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Chain of Dogs - V",
            icon: eventIcons.dog,
            description: "Onwards to Aren",
            char: [25, 26],
            loc: [53.98, 56.79],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Sha'ik Reborn",
            icon: eventIcons.prayer,
            description: "Felisin is enticed into becoming Sha'ik Reborn.",
            char: [19, 20],
            loc: [55.755, 47.585],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Tremorlor",
            icon: eventIcons.ruins,
            description: "The group enters Tremalore.",
            char: [23, 24, 28, 2, 12, 15],
            loc: [53.1, 46.05],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Chain of Dogs - VI",
            icon: eventIcons.dog,
            description: "The Silanda returns",
            char: [25, 26],
            loc: [53.985, 59.86],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Pirates?",
            icon: eventIcons.travelBoat,
            description: "Kalam is on the boat on board to Unta.",
            char: [11],
            loc: [65.93, 90.75],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Chain of Dogs - VII",
            icon: eventIcons.dog,
            description: "The Sapper Captain is demoted",
            char: [25, 26],
            loc: [49.56, 61.4],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Storm",
            icon: eventIcons.travelBoat,
            description: "They are semi-trapped inside a Warren and are blown of course to Malaz City.",
            char: [11],
            loc: [75.93, 100.75],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Chain of Dogs - VIII",
            icon: eventIcons.dog,
            description: "The merchants arrive bringing needed supplies",
            char: [25, 26],
            loc: [42.48, 67.53],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Chain of Dogs - IX",
            icon: eventIcons.dog,
            description: "Duiker is orderd to go to Aren with the refugees",
            char: [25],
            loc: [42.48, 73.67],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Chain of Dogs - X",
            icon: eventIcons.dog,
            description: "Outskirts of Aren",
            char: [25],
            loc: [46.905, 78.285],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Return to the city",
            icon: eventIcons.city,
            description: "Kalam arrives in Malaz City but is attacked by the Claw.",
            char: [11],
            loc: [76.995, 179.595],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Chain of Dogs - XI",
            icon: eventIcons.dog,
            description: "Duiker and the refugees enter Aren, High Fist Pormqual refuses to help and the rest of Coltaines army is killed.",
            char: [25, 26],
            loc: [47.79, 79.82],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Kalam",
            icon: eventIcons.assassin,
            description: "Kalam does Kalam stuff.",
            char: [11],
            loc: [76.995, 179.595],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Chain of Dogs - XII",
            icon: eventIcons.dog,
            description: "Duiker is betrayed and crucified outside Aren with the rest of the army.",
            char: [25],
            loc: [47.79, 79.82],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Deadhouse",
            icon: eventIcons.warren,
            description: "Fiddlers group end up in Malaz City by usign the Deadhouse gates.",
            char: [2, 12, 15],
            loc: [76.995, 179.595],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Mocks Hold",
            icon: eventIcons.assassin,
            description: "Kalam speaks to empress Lassen.",
            char: [10, 11],
            loc: [76.995, 179.595],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Sha'ik Reborn",
            icon: eventIcons.whirlwind,
            description: "Felisin uses the whirlwind to teleport to Aren gathers her rebellion army and returns to Raraku.",
            char: [19, 20],
            loc: [46.905, 78.285],
            moon: [179, 0, 70],
        },
        {
            id: uuidv4(),
            book: 2,
            name: "Aren way",
            icon: eventIcons.campfire,
            description: "Mappo and Icarium end up outside Aren and meet the Silandra crew who are seraching for Duikers body which they cant find..",
            char: [23, 24],
            loc: [46.905, 78.285],
            moon: [179, 0, 70],
        },
    ],
};

export default timelineData;
